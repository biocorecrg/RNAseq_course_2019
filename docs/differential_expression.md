...
* Remove lowly expressed genes: we want to remove genes that have no or very little expression. It will allow us to proceed with the statistical analysis using less genes. Working with less genes increases the **statistical power**.
  * Keep only those genes that have more than 10 summed raw counts across the 6 samples:
...
```{bash}
# compute normalized counts (log2 transformed); + 1 is a count added to avoid errors during the log2 transformation: log2(0) can't be computed, but log2(1) is 0.
